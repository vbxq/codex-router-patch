...
const managedSubagentCompletionHint =
  "After calling spawn_agent, keep the parent turn alive and do not emit task_complete. For long work call wait_agent with timeout_ms=3600000. If wait_agent returns timed_out=true, that is only an observation timeout, not completion: call wait_agent again and do not interrupt the child. Read every child report and verify the requested deliverables before concluding. A FINAL_ANSWER marked PARTIAL, in progress, incomplete, not delivered, or missing acceptance criteria is not a completed child: do not call interrupt_agent; send followup_task or send_message describing the missing work, then wait_agent again. Call interrupt_agent only after the child explicitly reports complete, or an idle/errored wait snapshot proves it cannot continue or is no longer needed. Only conclude when the overall task and every required deliverable are complete. If you need a same-thread follow-up, send it before any interrupt and wait for its result. Do not leave finished children in the working state.";
